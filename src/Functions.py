...
def get_distance_mask(L):
    """Return a distance mask matrix
    with shape (3, L, L) of sequence with length L.
    Calculated by m[i, j] = 1 / abs(i - j) ** p,
    p = (1, 2, 3)"""
    m = np.zeros((3, L, L))
...
def get_alt_structures(df):
    ...
    def preprocess_inputs(df, cols):
        return np.transpose(
            np.array(
                df[cols].map(lambda seq: [token2int[x] for x in seq]).values.tolist()
            ),
            (0, 2, 1),
        )
...
def get_errors(
    ...
    return np.transpose(np.array(df[cols].values.tolist()), (0, 2, 1))
...
def get_best_weights_from_fold(fold, top=5):
    ...
    for i in range(top):
        weights_path = "checkpoints_fold{}/epoch{}.ckpt".format(
            fold, history.epoch[top_epochs[i]]
        )
        print(weights_path)
        os.system(
            "cp {} best_weights/fold{}top{}.ckpt".format(weights_path, fold, i + 1)
        )
...
def mutate_rna_input(sequence, nmute=0.15):
...
def mask_rna_input(sequence, nmute=0.15):
    nmute = int(sequence.shape[1] * nmute)
    perm = torch.randperm(sequence.size(1))
    to_mutate = perm[:nmute]
    ...
    with torch.no_grad():
        for data in tqdm(dataset):
            ...
            val_acc += Metrics.accuracy(
                classification_predictions.cpu().numpy(), Y.cpu().numpy()
            )
            #     predictions.append(pred.cpu().numpy())
            # for vector in output:
            #     outputs.append(vector.cpu().numpy())
