...
def train_fold():
    ...
    train_indices, val_indices = get_train_val_indices(
        json, opts.fold, SEED=2020, nfolds=opts.nfolds
    )
    ...
    pl_dataloader = DataLoader(
        pl_dataset, batch_size=opts.batch_size, shuffle=True, num_workers=opts.workers
    )
    ...
    finetune_dataset = RNADataset(
        train_seqs[:n_train],
        train_labels[:n_train, :68],
        train_ids[:n_train],
        train_ew[:n_train, :68],
        opts.path,
        k=opts.kmers[0],
    ...
    model.load_state_dict(
        torch.load(f"{opts.weight_path}/pretrain_weights/epoch200.ckpt")
    )
